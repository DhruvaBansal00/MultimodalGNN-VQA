...
class ClevrExecutor:
    ...
    def __init__(self, train_scene_json, val_scene_json, vocab_json, *args, **kwargs):
        self.scenes = {
    ...
    def run(self, x, img_index, split, guess=False, debug=False, **kwargs):
        ...
        for k in range(len(x)):
            ...
            # import pdb
            # pdb.set_trace()
            if self.vocab['program_idx_to_token'][int(x[l-1])] == '<END>':
                length = l
        if length == 0:
            return 'error'
        ...
        #Debug:
        if debug:
            pred_prog_seq = utils.get_prog_from_seq(x, self.vocab)
            debug_q = kwargs.get('debug_q')
            print(f"DBG: q -> {debug_q}")
            print(f"pred_prog_seq: {pred_prog_seq}")
        is_count_question = False
        def is_count(x:np.ndarray):
            # xl = x.tolist()
            mod = x[1]
            mod_token = self.vocab['program_idx_to_token'][int(mod)]
            if mod_token == 'count':
                return True
            return False

        def is_matching_re_question(x):
            is_matching = False
            prog_seq = utils.get_prog_from_seq(x, self.vocab)
            m = re.search("^count->union.*(relate).*scene$")
            if m is not None and m.string.count('relate') < 2:
        ...
        for j in range(length):
            i = length - 1 - j
            ## Traversing the modules LIFO with <End>,..., M_0
            token = self.vocab['program_idx_to_token'][int(x[i])]
            if token == 'scene':
                if temp is not None:
                    ans = 'error'
                    break
                ...
                if token.startswith('same') or token.startswith('relate'):
                    # same(obj, scene), relate(obj, scene)
                    ans = module(ans, scene)
                elif token.startswith('union'):
                    scene1 = ans
                    scene2 = temp
                    if is_count_question:
                        print('Union: is_count_question -> need to check matching_re')
                        print(scene2)
                        Xs, edge_index, edge_attr = data_s.x, data_s.edge_index, data_s.edge_attr
                        xoi = Xs[-1]        # the last node.
                        data_s_nodes = data_s.num_nodes
                        edge_oi = edge_attr[10]
                    ans = module(scene1,scene2)
                else:
                    ans = module(ans, temp)
                if ans == 'error':
                    break
        ...
        print(f"Completed executing programs, ans= {ans}")
        ans = str(ans)
        if ans == 'error' and guess:
            logging.debug(f"ans == 'error'. Guessing answer. ")
            final_module = self.vocab['program_idx_to_token'][int(x[0])]
    ...
    def _print_debug_message(self, x):
        if type(x) == list:
            for o in x:
                logging.debug(self._object_info(o))
        elif type(x) == dict:
            logging.debug(self._object_info(x))
        else:
            logging.debug(x)
